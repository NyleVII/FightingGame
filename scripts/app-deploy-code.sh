...
ssh -i ~/.ssh/appserver.pem ec2-user@34.217.232.210 "killall node; mv app/client/assets assets; rm -r app; mkdir app; mkdir app/logs; tar -xzf app.tar.gz -C app; rm app.tar.gz; mv assets app/client/assets; cd app; npm install --production"